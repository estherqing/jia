...
tableEvent = {
  'click .check': function(e, a, item, index){
	comn.addTab({
	  title:"贷款详情",
	  href: "./Modal/customManage/customer/loanDetail.html?id="+item.projectId+"&loanApplyId="+item.relativeApplyId1+"&businessTypeCode=" + item.flowType+"&projectId="+item.projectId + "&space=LOAN&releventFlowNode=LOAN_DOCUMENT_CHECK&releventFlow=LOAN_DOCUMENT_CHECK" + "&loanFlag=2"
	});
  } 
};
