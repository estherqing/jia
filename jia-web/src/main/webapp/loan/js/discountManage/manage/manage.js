...
//贴息金额是否勾选百分比
$("[name='isPerCent']").change(function(){
    ...
    if(_this.is(":checked")){
        ...
        controller1.val("").prop("readonly",true);
...
function getDisPolicySchemeNpers(){
    var isPerCent1=$("#nper36").find("[name='isPerCent']").is(":checked")?{isPerCent:true}:{isPerCent:false};
    var isPerCent2=$("#nper24").find("[name='isPerCent']").is(":checked")?{isPerCent:true}:{isPerCent:false};
    var isPerCent3=$("#nper18").find("[name='isPerCent']").is(":checked")?{isPerCent:true}:{isPerCent:false};
    var isPerCent4=$("#nper12").find("[name='isPerCent']").is(":checked")?{isPerCent:true}:{isPerCent:false};
    var disPolicySchemeNpers=[$.extend($("#nper36").values(),isPerCent1), $.extend($("#nper24").values(),isPerCent2), $.extend($("#nper18").values(),isPerCent3), $.extend($("#nper12").values(),isPerCent4)];
                ...
                for(i=0;i<schemeNpers.length;i++){
                    ...
                    switch (nper){
                        case 1:
                            nper12.values(a);
                            if(a.isPerCent){
                                nper12.find('[name="isPerCent"]').prop("checked",true);
                            }
                            if(a.discountLimit){
                        ...
                        case 2:
                            nper18.values(a);
                            if(a.isPerCent){
                                nper18.find('[name="isPerCent"]').prop("checked",true);
                            }
                            if(a.discountLimit){
                                nper18.find('[name="discountLimit"]').prop("readonly",false);
                            }
                            break;
                        case 3:
                            nper24.values(a);
                            if(a.isPerCent){
                                nper24.find('[name="isPerCent"]').prop("checked",true);
                            }
                            if(a.discountLimit){
                                nper24.find('[name="discountLimit"]').prop("readonly",false);
                            }
                            break;
                        case 4:
                            nper36.values(a);
                            if(a.isPerCent){
                                nper36.find('[name="isPerCent"]').prop("checked",true);
                            }
                            if(a.discountLimit){
                                nper36.find('[name="discountLimit"]').prop("readonly",false);
                            }
                            break;
                        default :
                            console.warn("没有期数数据");
