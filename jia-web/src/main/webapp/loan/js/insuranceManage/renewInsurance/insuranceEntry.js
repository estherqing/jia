...
if(type === "add") {
	...
	save_Values (interUrl.insurance.addInsuranceRenew);
	//获取当前操作用户
	//getUser();
	...
	comn.ajax({
		url: interUrl.insurance.getInsuranceRenew,
		data: {
			id: postData.id
		},
		success: function (res) {
			...
			for (var i = 0; i < res.data.loanInsuranceRenewalDetailList.length; i += 1) {
                    var item = res.data.loanInsuranceRenewalDetailList[i];
                    //判断字段是否为空
                    ...
                    htmlTemp += "<tr>" +
                    				...
                    				"<td data-name='index'><span>" + (i + 1) +
			                    		"</span><input type='hidden' class='insuranceTypeId' name='loanInsuranceRenewalDetailList[" + i + "].insuranceTypeId' date-rale='"+ item.id +"' value='" + item.insuranceTypeId + "'/>" +
			                    	"</td>" +
			                    	"<td class='id hidden'>" + item.insuranceTypeId + "</td>" +
			                   		"<td>" + item.insuranceTypeName +
			                   			"<input type='hidden' class='insuranceName' name='loanInsuranceRenewalDetailList[" + i + "].insuranceTypeName' value='" + item.insuranceTypeName + "'/>" +
			                   		"</td>" +
			                   		"<td>" +
			                   			"<input type='text' class='insuranceFee number' aria-required='true' onkeyup='value=moneyTest(value)' name='loanInsuranceRenewalDetailList[" + i + "].insuranceFee' value='" + insuranceFee + "'/>" +
			                   		"</td>" +
			                   		"<td>" +
			                   			"<input type='text' class='insuranceMoney' onkeyup='value=moneyTest(value)' name='loanInsuranceRenewalDetailList[" + i + "].insuranceMoney' value='" + insuranceMoney + "'/>" +
			                   		"</td>" +
	...
	save_Values (interUrl.insurance.modifyInsuranceRenew);
}
...
$("#insuranceInfoTable").on("click", ".removeLoanInsuranceType", function () {
    ...
    $("#insuranceInfoTable tbody tr").each(function (index) {
        ...
        $(this).find("input.insuranceTypeId").attr("name", "loanInsuranceRenewalDetailList[" + index + "].insuranceTypeId");
        $(this).find("input.insuranceName").attr("name", "loanInsuranceRenewalDetailList[" + index + "].insuranceTypeName");
        $(this).find("input.insuranceFee").attr("name", "loanInsuranceRenewalDetailList[" + index + "].insuranceFee");
        $(this).find("input.insuranceMoney").attr("name", "loanInsuranceRenewalDetailList[" + index + "].insuranceMoney");
    });
    var insuranceFeeTotal = getTotalInsuranceFee();
    $("#insuranceFeeTotal").val(insuranceFeeTotal);
});
    ...
    for (var i = 0; i < ids.length; i += 1) {
        ...
        htmlTemp += "<tr>" + 
        				...
        				"<td data-name='index'><span>" + lenIndex +
                    		"</span><input type='hidden' class='insuranceTypeId' name='loanInsuranceRenewalDetailList[" + len + "].insuranceTypeId'" +"' value='" + ids[i] + "'/>" +
                    	"</td>" +
                    	"<td class='id hidden'>" + ids[i] + "</td>" +
                   		"<td>" + names[i] +
                   			"<input type='hidden' class='insuranceName' name='loanInsuranceRenewalDetailList[" + len + "].insuranceTypeName' value='" + names[i] + "'/>" +
                   		"</td>" +
                   		"<td>" +
                   			"<input type='text' class='insuranceFee number' aria-required='true' onkeyup='value=moneyTest(value)'  name='loanInsuranceRenewalDetailList[" + len + "].insuranceFee' />" +
                   		"</td>" +
                   		"<td>" +
                   			"<input type='text' class='insuranceMoney' onkeyup='value=moneyTest(value)' name='loanInsuranceRenewalDetailList[" + len + "].insuranceMoney' />" +
                   		"</td>" +
...
// 			                   		"</td>" +
//                     				"<td data-name='index'><span>" + lenIndex +
// 			                    		"</span><input type='hidden' class='insuranceTypeId' name='loanInsuranceRenewalDetailList[" + len + "].insuranceTypeId'" + "' value='" + item.id + "'/>" +
// 			                    	"</td>" +
// 			                    	"<td class='id hidden'>" + item.id + "</td>" +
// 			                   		"<td>" + item.insuranceName +
// 			                   			"<input type='hidden' class='insuranceName' name='loanInsuranceRenewalDetailList[" + len + "].insuranceTypeName' value='" + item.insuranceName + "'/>" +
// 			                   		"</td>" +
// 			                   		"<td>" +
// 			                   			"<input type='text' class='insuranceFee money_credit' aria-required='true' name='loanInsuranceRenewalDetailList[" + len + "].insuranceFee' />" +
// 			                   		"</td>" +
// 			                   		"<td>" +
// 			                   			"<input type='text' class='insuranceMoney' aria-required='true' name='loanInsuranceRenewalDetailList[" + len + "].insuranceMoney' />" +
// 			                   		"</td>" +
