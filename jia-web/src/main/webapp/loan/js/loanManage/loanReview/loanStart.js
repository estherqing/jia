...
//贷款申请时提交按钮变更为提交签单调度
if(args["releventFlow"] === "LOAN_APPLY_FLOW") $("#submit span").html("提交签单调度");
...
function repaymentAmount() {   //试算月还款额=【贷款金额*(1+手续费率)-车商贴息政策】/贷款期限(月)    业务品种=4时,试算月还款额=贷款金额*(1+公司手续费率)/期限(月)
    var repaymentAmount,
        loanTermVal = $("select[name='loanTerm']").val(),   //贷款期限
        loanAmount = $("input[name='loanAmount']").val() || 0,   //贷款金额
        handingFee = $("input[name='handingFee']").val() || 0,   //手续费率
        discountCarPolicy = $("input[name='discountCarPolicy']").val() || 0,//贴息政策
        businessTypeId = $("#product").val(); //业务品种
    if (loanTermVal !== "" && !isNaN(loanAmount) && !isNaN(handingFee)) {
        if (loanTermVal == 1) loanTerm = 12;
        if (loanTermVal == 2) loanTerm = 18;
        if (loanTermVal == 3) loanTerm = 24;
        if (loanTermVal == 4) loanTerm = 36;
        if (businessTypeId == 4) {
            repaymentAmount = (parseFloat(loanAmount) * (1 + parseFloat(handingFee) / 100)) / loanTerm;
        } else {
            repaymentAmount = (parseFloat(loanAmount) * (1 + parseFloat(handingFee) / 100) - parseFloat(discountCarPolicy)) / loanTerm;
        }
        $("input[name='repaymentAmount']").val(repaymentAmount.toFixed(2));
    }
}
    ...
    comn.ajax({
        ...
        success: function (res) {
            if (res.data) {
                ...
                for (var i = 0; i < length; i++) {
                    html += '<option value="' + res.data[i].id + '" data-nper="' + res.data[i].nper + '">' + res.data[i].productName + '</option>'
                }
                $("#productId").html(html);
            }
            ;
...
$(function () {
    ...
    $(document).on("click", ".loanStart3", function () {
        ...
        var _url = flagSubmit ? (args["releventFlow"]==="LOAN_MODIFY_FLOW" ? interUrl.loanModify.submit2NextByNodeKey: interUrl.loanDetail.pre2NextByNodeKey) : submit2nextURL;
        var _data1 = {
            loanApplyId: a,
            nextNodeUserId: b,
            nextNodeUserName: c
        ...
        flagSubmit ? _data1.nodeKey = (args["releventFlow"] === "LOAN_MODIFY_FLOW" ? "LOAN_MODIFY_APPROVE_OFFICE_STAFF":"LOAN_APPROVE_OFFICE_STAFF") : _data1;
        comn.ajax({
    ...
    function isNeedDiscount() {
        var carType = $("#carType").val();
        //var getCarModel=$("#getCarModel").val();
        if (isSupportDiscount === 1 && carType === "1") {
            return true;
        } else {
            return false;
        }
    ...
    $("input[name='loanAmount'],input[name='handingFee'],input[name='discountAmount']").keyup(function () {
        repaymentAmount();
    });
