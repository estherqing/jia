var table_pay,tableEvent_pay,handle_pay,table_pay1,tableEvent_pay1,handle_pay1,choosePay=null,flowType,conclusion,discountCaseId, _name, _id;
...
comn.ajax({
  ...
  success: function(res) {
    _id = res.data.dealerId;
    _name = res.data.dealerName;
    ...
    if(res.data.dealerId){
      $("#payModal1").on("shown.bs.modal", function() {
        ...
        handle_pay1 = function (value, row, index) {
          return ["<a class='pay' href='javascript:;'>选择</a>"].join("");
        };
        $("#table_pay1").bootstrapTable("refresh");
      })
    }
      }
});
            ...
            comn.ajax({
              ...
              success: function (res) {
                if($('#payType').find('option:selected').val()==1){
                  flowSubmit(interUrl.loanModify.preSubmit, interUrl.loanModify.submit2next, './Modal/task/myTask/index.html', loanApplyId,"RA");
                }else{
...
$(function () {
  $("#userSearch").click(function() {
    return $("#table_pay1").bootstrapTable('selectPage', '1');
  });
})
