    ...
    if(name){
        ...
        if(cookie.length > 0){
            ...
            if(start != -1){
                ...
                if(end != -1){
                    cookie = cookie.substr(start,end - start);
                    return cookie;
                }
...
if (needCLodop()) {
    ...
    oscript.src ="http://localhost:"+ (getCookie('printPort') || 8000) +"/CLodopfuncs.js?priority=1";
    if(document.location.protocol == 'https:'){
        oscript.src ="https://localhost:" + (getCookie('printPort') || 8443) + "/CLodopfuncs.js?priority=1";
    }
    var head = document.head || document.getElementsByTagName("head")[0] || document.documentElement;
    head.insertBefore( oscript,head.firstChild );
};
